"""Build the v0.2.2 coder-facing retest package.
...
PACKAGE_NAME = "TRIM_retest_v0_2_2_coder_package.zip"
...
def main() -> int:
    ...
    sums_path = outdir / "TRIM_retest_v0_2_2_coder_package.SHA256SUMS.txt"
    audit_json_path = outdir / "TRIM_retest_v0_2_2_semantic_steering_audit.json"
    audit_text_path = outdir / "TRIM_retest_v0_2_2_semantic_steering_audit.txt"
...
def _assert_manifest_integrity(root: Path) -> None:
    manifest = _read_csv(root / "data" / "retest_v0_2_2_case_manifest.csv")
    registry = _read_csv(root / "data" / "retest_v0_2_2_shared_context_registry.csv")
    provenance = _read_csv(root / "data" / "retest_v0_2_2_source_text_provenance.csv")
    source_packet = (root / "data" / "retest_v0_2_2_source_packet.md").read_text(
        encoding="utf-8"
    )
    issues = [
...
def semantic_steering_audit(root: Path) -> dict[str, object]:
    ...
    source_quote_lines = _source_quote_lines(
        (root / "data" / "retest_v0_2_2_source_packet.md").read_text(
            encoding="utf-8"
        )
    )
...
def _write_steering_reports(
    ...
    lines = [
        "TRIM v0.2.2 semantic-steering audit",
...
def _source_quote_lines(source_packet_text: str) -> dict[tuple[str, int], str]:
    ...
    file_path = "data/retest_v0_2_2_source_packet.md"
