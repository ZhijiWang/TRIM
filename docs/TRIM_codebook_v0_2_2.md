...
This codebook defines the pilot-informed v0.2.2 annotation fields, controlled
vocabularies, evidence-selection model, and review rules for TRIM:
Threshold-Rationale Interpretive Modelling.

v0.2.2 is a prospective retest design. The v0.2.0 ten-case usability pilot
...
For the v0.2.2 retest, `function_label` is a closed project-specific list:
...
v0.2.2 makes evidence selection discriminative.

| Field | Status | Rule |
| --- | --- | --- |
| `primary_evidence_segment_ids` | required for v0.2.2 coding | one to three segment IDs |
...
does not satisfy the v0.2.2 evidence-selection task unless the rationale
...
When present, `rationale_note` must explain the competing pathway. v0.2.2
...
If an interpretive or definitional question changes the final code, but the
final annotation uses `uncertainty_flag=low` and no complete
`alternative_signature`, validation emits a warning for review. This warning
...
`cue_family` and `broad_function_family` remain supported for backward
compatibility and researcher-side description of older datasets, but they are
not coder-generated v0.2.2 retest fields. They are excluded from the v0.2.2
coder template, are not required for coder submission, and should not be used in
field-level intercoder agreement calculations.

## Language Access

v0.2.2 records language access instead of assuming original-language
reproducibility.
...
summary-mediated usability pilot. For v0.2.2, either recruit coders who can read
the source languages directly or explicitly standardize translation-mediated
conditions.

For v0.2.2 formal retest coding, coder-facing segments supply actual source
...
v0.2.2 cases must state whether coders may use only a local passage, a
...
`data/retest_v0_2_2_shared_context_registry.csv`, with one row per
...
v0.2.2 fields or status. v0.2.2 retest records require
